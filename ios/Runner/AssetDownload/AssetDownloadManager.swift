...
final class AssetDownloadManager {
  ...
  private let workQueue = DispatchQueue(label: "com.rnr.deenfocus.assetdownload.manager", qos: .utility)
  ...
  /// the same pack version). Always performs network + file I/O on an internal
  /// background queue — safe to call from any thread, including the main thread.
  func downloadFiles(
    _ specs: [FileSpec],
    into stagingDir: URL,
    ...
    workQueue.async { [weak self] in
      ...
      do {
        try self.downloadFilesSync(specs, into: stagingDir, isCancelled: isCancelled, progress: progress)
      } catch {
        thrown = error
      }
      semaphore.signal()
